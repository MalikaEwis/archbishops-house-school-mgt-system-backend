...
function toDateInput(val) { return val ? String(val).slice(0, 10) : ''; }
function toStr(val)       { return val == null ? '' : String(val); }
function strOrNull(val)   { const t = String(val ?? '').trim(); return t === '' ? null : t; }
...
export default function VestedSchoolDetailPage() {
  ...
  function openEditPrincipal(p) {
    setPrincipalForm({
      ...
      appointment_to_present_school: toDateInput(p.appointment_to_present_school),
  ...
  async function savePrincipal() {
    ...
    if (dob && ret && ret < dob) {
      setPrincipalError('Retirement date cannot be earlier than date of birth.');
      return;
    }
    if (dob && fad && fad < dob) {
      setPrincipalError('First appointment date cannot be earlier than date of birth.');
      return;
    }
    if (fad && atps && atps < fad) {
      setPrincipalError('Appointment to this school cannot be earlier than first appointment date.');
      return;
    }
    ...
    try {
      const body = {
        ...
        appointment_to_present_school: principalForm.appointment_to_present_school || null,
      ...
      setPrincipalError(err.response?.data?.message ?? 'Failed to save principal.');
    } finally {
  ...
  async function saveArchive() {
    ...
    try {
      ...
      setPrincipalError(err.response?.data?.message ?? 'Failed to archive principal.');
    } finally {
      setPrincipalSaving(false);
    }
  }
  ...
  async function saveStats() {
    ...
    if (statsEditYear == null) {
      ...
      if (exists) {
        setStatsError(`Statistics for ${year} already exist. Use Edit to update.`);
        return;
      }
    }
    const COUNT_FIELDS = [
    ...
    try {
      ...
      setStatsError(err.response?.data?.message ?? 'Failed to save statistics.');
    } finally {
  ...
  async function handleDeleteStats(year) {
    if (!window.confirm(`Delete statistics for ${year}? This cannot be undone.`)) return;
    setStatsError('');
    try {
      ...
      setStatsError(err.response?.data?.message ?? 'Failed to delete statistics.');
    }
  }
    ...
    <div className={detailStyles.page}>
      ...
      <section className={detailStyles.section}>
        ...
        {isAdmin && (principalMode === 'add' || principalMode === 'edit') && (
          <div style={INLINE_FORM}>
            ...
            {principalError && (
              <p className={detailStyles.error} style={{ marginBottom: '0.75rem' }}>
                {principalError}
              </p>
            )}
            ...
            <div style={TWO_COL}>
              ...
              <FormField label="NIC">
                <input
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, nic: e.target.value }))}
                  disabled={principalSaving}
              ...
              <FormField label="Gender">
                <select
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, gender: e.target.value }))}
                  disabled={principalSaving}
                >
              ...
              <FormField label="Religion">
                <input
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, religion: e.target.value }))}
                  disabled={principalSaving}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setPrincipalForm((p) => ({
                      ...
                      retirement_date: p.retirement_date || calcRetirementDate(dob),
                    }));
                  }}
                  disabled={principalSaving}
                />
              ...
              <FormField label="Retirement date">
                <input
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, retirement_date: e.target.value }))}
                  disabled={principalSaving}
                />
              </FormField>
              ...
              <FormField label="First appointment date">
                <input
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, first_appointment_date: e.target.value }))}
                  disabled={principalSaving}
                />
              </FormField>
              ...
              <FormField label="Appointed to this school">
                <input
                  ...
                  onChange={(e) => setPrincipalForm((p) => ({ ...p, appointment_to_present_school: e.target.value }))}
                  disabled={principalSaving}
                />
              </FormField>
            ...
            <div style={TWO_COL}>
              <FormField label="End date">
                <input
                  ...
                  onChange={(e) => setArchiveForm((p) => ({ ...p, end_date: e.target.value }))}
                  disabled={principalSaving}
                />
              </FormField>
              ...
              <FormField label="Departure reason">
                <input
                  ...
                  onChange={(e) => setArchiveForm((p) => ({ ...p, departure_reason: e.target.value }))}
                  disabled={principalSaving}
      ...
      <section className={detailStyles.section}>
        ...
        {statsMode === null && statsError && (
          <p className={detailStyles.error} style={{ marginBottom: '1rem' }}>{statsError}</p>
        )}
        ...
        {isAdmin && statsMode === 'form' && (
          <div style={INLINE_FORM}>
            <p style={FORM_TITLE}>
              {statsEditYear != null ? `Edit Statistics — ${statsEditYear}` : 'Add Statistics'}
            </p>
            {statsError && (
              <p className={detailStyles.error} style={{ marginBottom: '0.75rem' }}>{statsError}</p>
            )}
            ...
            <div style={THREE_COL}>
              <FormField label="Year *">
                <input
                  ...
                  onChange={(e) => setStatsForm((p) => ({ ...p, stat_year: e.target.value }))}
              ...
              <FormField label="Total teachers">
                <input
                  ...
                  onChange={(e) => setStatsForm((p) => ({ ...p, total_teachers: e.target.value }))}
              ...
              <FormField label="Total classes">
                <input
                  ...
                  onChange={(e) => setStatsForm((p) => ({ ...p, total_classes: e.target.value }))}
                  disabled={statsSaving}
                  min="0"
                />
              </FormField>
            ...
            {/* Religion counts */}
            <p style={{ fontSize: '0.78rem', fontWeight: 600, color: '#6b7280', margin: '0 0 0.5rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              ...
              {[
                ...
                <FormField key={name} label={label}>
                  <input
                    ...
                    onChange={(e) => setStatsForm((p) => ({ ...p, [name]: e.target.value }))}
                    disabled={statsSaving}
                    min="0"
                  />
                </FormField>
            ...
            <p style={{ fontSize: '0.78rem', color: '#6b7280', margin: '-0.35rem 0 0.75rem' }}>
        ...
        {/* Stats table */}
        {stats.length === 0 ? (
          statsMode === null && <p className={detailStyles.stateMsg}>No student statistics recorded.</p>
        ) : (
          <div className={listStyles.tableWrap}>
            <table className={listStyles.table}>
              <thead>
              ...
              <tbody>
                {stats.map((st) => (
                  <tr key={st.stat_year}>
                    ...
                    <td>{fmtStatPct(st.count_other_christian, st.total_students)}</td>
                    <td>{fmtStatPct(st.count_other_religion,  st.total_students)}</td>
                    <td>{fmtStatPct(st.count_sinhala_medium,  st.total_students)}</td>
                    <td>{fmtStatPct(st.count_tamil_medium,    st.total_students)}</td>
                    <td>{fmtStatPct(st.count_english_medium,  st.total_students)}</td>
                    {isAdmin && (
                      <td style={{ whiteSpace: 'nowrap' }}>
                        <button
                          className={detailStyles.editBtn}
                          style={{ padding: '0.2rem 0.55rem', fontSize: '0.78rem', marginRight: '0.3rem' }}
